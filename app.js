...
(function () {
  ...
  var sS = 6, sE = 22;
  var todos = [];
  var pTexts = ['', '', ''];   // 핵심 3가지 텍스트
  var pDone  = [false, false, false]; // 핵심 3가지 달성
  var blocks = [], bId = 0;
  ...
  var toastTimer = null;
  function toast(m) {
    var t = qs('#toast'); t.textContent = m; t.classList.add('show');
  ...
  function buildPrioContainer(containerId, withInput) {
    ...
    for (var i = 0; i < 3; i++) {
      (function(idx){
        var row = ce('div'); row.className = 'pi' + (pDone[idx] ? ' dp' : ''); row.id = 'pi-' + idx + '-' + containerId;

        var cb = ce('div'); cb.className = 'pcb' + (pDone[idx] ? ' on' : ''); cb.id = 'pcb-' + idx + '-' + containerId;
  ...
  function refreshSchedPrio() {
    for (var i = 0; i < 3; i++) {
      ...
      ['todo','sched'].forEach(function(cid){
        var cb   = qs('#pcb-'+ i +'-plist-'+ cid);
        var item = qs('#pi-' + i +'-plist-'+ cid);
        if (!cb || !item) return;
  ...
  function renderTodos() {
    ...
    todos.forEach(function(t, i){
      ...
      // 텍스트
      var sp = ce('span'); sp.textContent = t.text; sp.className = 'ti-txt';
      // 액션 버튼 그룹
      var actions = ce('div'); actions.className = 'ti-actions';
      // 수정 버튼
      var editBtn = ce('button'); editBtn.className = 'ti-btn edit'; editBtn.innerHTML = '✏️'; editBtn.title = '수정';
      editBtn.addEventListener('click', function(e){
        e.stopPropagation();
        var newTxt = prompt('할 일 수정:', t.text);
        if (newTxt !== null && newTxt.trim()) { todos[i].text = newTxt.trim(); renderTodos(); saveStorage(); syncToday(); }
      });
      // 삭제 버튼
      var delBtn = ce('button'); delBtn.className = 'ti-btn del'; delBtn.innerHTML = '🗑️'; delBtn.title = '삭제';
  ...
  function showReceiptView() {
    ...
    // prios
    var pc = qs('#rv-prios'); pc.innerHTML = '';
    var pDoneCount = 0;
    pTexts.forEach(function(v, i){
      if (!v) return; if (pDone[i]) pDoneCount++;
    ...
    var totalPrios = pTexts.filter(function(t){ return t; }).length;
  ...
  function saveReceiptImage() {
    ...
    var area = qs('#receipt-area'), signCard = qs('.sign-card');
    var wrapper = ce('div');
    ...
    var aClone = area.cloneNode(true);
    var sClone = signCard.cloneNode(true);
    var origCv = qs('#scv'), cloneCv = sClone.querySelector('canvas');
    if (cloneCv && origCv) { cloneCv.width=origCv.width; cloneCv.height=origCv.height; cloneCv.getContext('2d').drawImage(origCv,0,0); }
    sClone.querySelectorAll('button').forEach(function(b){ b.style.display='none'; });
    wrapper.appendChild(aClone); wrapper.appendChild(sClone); document.body.appendChild(wrapper);
    setTimeout(function(){
      html2canvas(wrapper,{backgroundColor:'#f2f2f2',scale:2,useCORS:true,allowTaint:true,logging:false,scrollX:0,scrollY:0})
      .then(function(c){
        document.body.removeChild(wrapper); btn.textContent='⬇ 이미지 저장'; btn.disabled=false;
        if (c.toBlob) { c.toBlob(function(blob){ var url=URL.createObjectURL(blob); dl(url,'diary-'+new Date().toISOString().slice(0,10)+'.png'); setTimeout(function(){ URL.revokeObjectURL(url); },1000); },'image/png'); }
        else dl(c.toDataURL('image/png'),'diary-'+new Date().toISOString().slice(0,10)+'.png');
        toast('PNG 저장 완료! 📸');
      }).catch(function(err){ document.body.removeChild(wrapper); btn.textContent='⬇ 이미지 저장'; btn.disabled=false; console.error(err); toast('저장 실패'); });
    },100);
  }
  ...
  /* ══════════════════════════════ 서명 ══════════════════════════════ */
  function initSign() {
    var previewCard = qs('#sign-preview-card');
    var previewCv   = qs('#scv-preview');
    if (previewCard) previewCard.addEventListener('click', openSignModal);
    if (previewCv)   previewCv.addEventListener('click', openSignModal);
  }
  ...
  function openSignModal() {
    ...
    setTimeout(function(){
      ...
      var w = cv.parentElement.clientWidth || 360, h = 200;
      // 새 canvas로 교체 (이벤트 중복 방지)
  ...
  function closeSignModal(save) {
    if (save) {
      ...
      if (src && dst) {
        var dpr = window.devicePixelRatio||1;
        dst.width  = dst.offsetWidth  * dpr;
        dst.height = dst.offsetHeight * dpr;
        dst.getContext('2d').drawImage(src, 0, 0, dst.width, dst.height);
        var hint = qs('#sign-preview-hint'); if (hint) hint.style.display='none';
      }
    }
  ...
  function attachSchedEvents() {
    ...
    cv.addEventListener('drop', function(e){
      e.preventDefault(); var txt=e.dataTransfer.getData('text/plain')||dragTxt; if(!txt)return;
      var r=cv.getBoundingClientRect(), sh=snap(Math.max(sS,Math.min(sE-0.5,(e.clientY-r.top)/ROW+sS)));
  ...
  function bindPWA() {
    ...
    window.addEventListener('beforeinstallprompt',function(e){ e.preventDefault(); deferredInstall=e; var btn=qs('#btn-pwa-install'); if(btn){btn.disabled=false;btn.textContent='설치';} });
    var installBtn=qs('#btn-pwa-install');
    if(installBtn){ installBtn.addEventListener('click',function(){ if(deferredInstall){deferredInstall.prompt();deferredInstall.userChoice.then(function(r){deferredInstall=null;if(r.outcome==='accepted'){toast('앱 설치 중... 🎉');setInstalledState();}else{var st=qs('#install-status');if(st)st.textContent='설치를 취소했습니다.';} });}else{toast('아래 수동 설치 방법을 따라주세요 👇');} }); }
    window.addEventListener('appinstalled',function(){toast('앱 설치 완료! 🎉');setInstalledState();});
  }
  function setInstalledState(){var p=qs('#pwa-install-popup');var b=qs('#installed-banner');var st=qs('#install-status');if(p)p.style.display='none';if(b)b.classList.add('show');if(st)st.textContent='홈 화면에서 앱을 실행할 수 있습니다.';}

  /* ══════════════════════════════ 폭죽 ══════════════════════════════ */
  function fireConf(){
    var cv=qs('#conf-cv');cv.style.display='block';cv.width=window.innerWidth;cv.height=window.innerHeight;
    var ctx=cv.getContext('2d');var cols=['#f9a8c9','#7ec8e3','#90e0af','#ffd580','#b5a0f7','#e74c3c'];
    var ps=[];for(var i=0;i<90;i++)ps.push({x:Math.random()*cv.width,y:cv.height*.4,vx:(Math.random()-.5)*10,vy:-(Math.random()*10+4),r:Math.random()*4+2,c:cols[i%cols.length],rot:Math.random()*360,rv:(Math.random()-.5)*12,life:1});
